...
const { incomeText, expensesText, income, expenses, wallet, currencyFormated } =
  props
const { generateTransactionData, generateEmptyMonthData, formatedPrice } =
  useUtils()
...
function useAreaExpenses() {
      ...
      y: {
        formatter: function (val: number) {
          return currencyFormated
            ? formatedPrice(val, wallet.currency)
            : val + ' ' + wallet.currency
        },
      },
    },
  ...
  const series = computed(() => {
    const [incomeData, expenseData] = wallet
      ? generateTransactionData(wallet.transactions as Transaction[])
...
<template>
  <BaseCard
    class="!bg-muted-200 dark:!bg-muted-800 gap-y-10 p-6 col-span-12 lg:col-span-8"
  >
          ...
          <div class="p-4">
            ...
            <div class="text-success-500 font-sans text-sm font-semibold">
              <span
                ...
                {{
                  currencyFormated
                    ? formatedPrice(income, wallet.currency)
                    : income + ' ' + wallet.currency
                }}</span
              >
            </div>
          </div>
          <div class="p-4">
            ...
            <div class="text-danger-500 font-sans text-sm font-semibold">
              <span
                ...
                {{
                  currencyFormated
                    ? formatedPrice(expenses, wallet.currency)
                    : expenses + ' ' + wallet.currency
                }}</span
              >
            </div>
          </div>
