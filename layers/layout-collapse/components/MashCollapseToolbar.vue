...
const showNavBurger = computed(() => {
  return props.collapse && app.mash.collapse?.toolbar?.showNavBurger
})
</script>
...
<template>
  ...
  >
    <MashCollapseBurger v-if="showNavBurger" class="-ms-3" />

    <BaseHeading
      v-if="app.mash.collapse?.toolbar?.showTitle"
      as="h1"
      size="2xl"
      weight="light"
    ...
    <template v-for="tool of app.mash.collapse?.toolbar?.tools">
      <component
