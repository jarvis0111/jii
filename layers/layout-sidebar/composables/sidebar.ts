...
export function useSidebar() {
  ...
  const sidebars = computed(() => {
    if (
      app.mash.sidebar?.navigation?.enabled === false ||
      app.mash.sidebar?.navigation?.items?.length === 0
    ) {
      return []
    }
    return app.mash.sidebar?.navigation?.items
  })
  ...
  function detect() {
    if (!app.mash.sidebar?.navigation?.startOpen) {
      isOpen.value = false
      return
    }
