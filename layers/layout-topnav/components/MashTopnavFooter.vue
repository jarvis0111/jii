...
const config = useAppConfig().mash.topnav?.footer
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <span
          v-if="appConfig?.public.siteName && appConfig?.public.siteUrl"
          class="inline-flex gap-1"
        >
          <span>©</span>
          <NuxtLink
            :to="appConfig?.public.siteUrl"
            target="_blank"
            rel="noopener"
            class="text-muted-600 hover:text-primary-500 dark:text-muted-200 dark:hover:text-primary-400 text-sm transition-colors duration-300"
          >
            {{ appConfig?.public.siteName }}
          </NuxtLink>
          <span>
            {{ year }}
          </span>
